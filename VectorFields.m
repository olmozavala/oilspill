classdef VectorFields
  ...
  methods
    ...
    function obj = readUV(obj,fixedWindDeltaT,modelHour,modelDay)
      ...
      atmFilePrefix    = 'WRF_';        % File prefix for the atmospheric netcdf files
      oceanFilePrefix  = 'hycom_2010_'; % File prefix for the ocean netcdf files
      ...
      readOceanFile    = [oceanFilePrefix,sprintf('%03d',modelDay),'_00_3z.nc'];
      readOceanFileT2  = [oceanFilePrefix,sprintf('%03d',modelDay+1),'_00_3z.nc'];
      readWindFile     = [atmFilePrefix,num2str(modelDay),'_',num2str(windFileNum),'.nc'];
      ...
      if readNextDayWind
        readWindFileT2 = [atmFilePrefix,num2str(modelDay+1),'_1.nc'];
        obj.UW         = obj.UWT2;
        obj.VW         = obj.VWT2;
      else
        readWindFileT2 = [atmFilePrefix,num2str(modelDay),'_',num2str(windFileT2Num),'.nc'];
      end
      ...
      if readOcean
        %readOceanFile
        obj.U = double(ncread(readOceanFile,'u',[1, 1, idx_depth_1 1],[Inf, Inf, 1 1])');
        obj.V = double(ncread(readOceanFile,'v',[1, 1, idx_depth_1 1],[Inf, Inf, 1 1])');
      end
      % Verify if we need to read the currents of the next day
      if readOceanT2
        %readOceanFileT2
        obj.UT2 = double(ncread(readOceanFileT2,'u',[1, 1, idx_depth_1 1],[Inf, Inf, 1 1])');
        obj.VT2 = double(ncread(readOceanFileT2,'v',[1, 1, idx_depth_1 1],[Inf, Inf, 1 1])');
      end
